...
from .models import ProductionRun, ProductionReport, ManufacturingOrder, ProductionLine
...
class ProductionCalculationService:
    ...
    def get_top_downtime_reasons(start_date: date, end_date: date, 
        ...
        downtime_analysis = queryset.values('code', 'reason').annotate(
    ...
    def generate_production_efficiency_report(start_date: date, end_date: date, 
        ...
        base_data['production_summary'] = queryset.aggregate(
            total_production=Sum('good_products_pack'),
            total_planned_production=Sum('order_number__quantity'),
            total_downtime=Sum('total_downtime_minutes'),
            avg_oee=Avg('report__oee'),
            total_runs=Count('id')
        )
        ...
        if base_data['production_summary']['total_planned_production']:
            fulfillment_rate = (
                base_data['production_summary']['total_production'] / 
                base_data['production_summary']['total_planned_production'] * 100
            )
            base_data['production_summary']['fulfillment_rate'] = round(fulfillment_rate, 2)
